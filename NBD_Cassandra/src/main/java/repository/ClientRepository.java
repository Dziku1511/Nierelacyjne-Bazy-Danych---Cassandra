...
public class ClientRepository extends Repository{
    ...
    public void addClient(Client client){
       session.execute("INSERT INTO client (clientId, firstName, lastName) VALUES (?,?,?)", client.getClientId(),
               client.getFirstName(), client.getLastName());
    }

    public void removeClient(UUID personalId) {
       session.execute("DELETE FROM client WHERE clientId = ?", personalId);
    }

    public void update(Client client) {
        session.execute("UPDATE client SET firstName = ?, lastName = ? WHERE clientId = ?", client.getFirstName(),
                client.getLastName(), client.getClientId());
    }

    public ResultSet getClient(UUID personalId) {
        return session.execute("SELECT * FROM client WHERE clientId = ?", personalId);
    }
